...
@router.post("/add", response_model=Dict[int, int], status_code=status.HTTP_200_OK)
...
@router.put("/update", response_model=Dict[int, int], status_code=status.HTTP_200_OK)
def update_cart_item(request: UpdateCartItemRequest, db: Session = Depends(get_db)):
    service = CartService(db)
...
@router.delete("/remove/{product_id}", response_model=Dict[int, int], status_code=status.HTTP_200_OK)
